...
AES is a block cipher that uses a Substitution Permutation Network and operates on bytes, instead of bits. The block size is 128 bit.
...
- Each byte is changes via a S-box
- Rows are shifted
- Columns are mixed with Matrix multiplication
- Each byte is xor-ed with the derived key
